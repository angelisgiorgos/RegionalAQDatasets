...
def magnitude_warp(x, sigma=0.2, knot=4):
    ...
    for i, pat in enumerate(x):
        warper = np.array(
            [CubicSpline(warp_steps[:, dim], random_warps[i, :, dim])(orig_steps) for dim in
             range(x.shape[2])]).T
        ret[i] = pat * warper
...
def time_warp(x, sigma=0.2, knot=4):
    ...
    for i, pat in enumerate(x):
        for dim in range(x.shape[2]):
            ...
            ret[i, :, dim] = np.interp(orig_steps, np.clip(scale * time_warp, 0, x.shape[1] - 1),
                                       pat[:, dim]).T
    return ret
    ...
    for i, pat in enumerate(x):
        for dim in range(x.shape[2]):
            ...
            window_seg = np.interp(
                np.linspace(0, warp_size - 1, num=int(warp_size * warp_scales[i])), window_steps,
    ...
    for i, pat in enumerate(x):
        ...
        if choices.size > 0:
            ...
            path2 = dtw.dtw(pat[random_points[i]:], random_sample[random_points[i]:],
                            dtw.RETURN_PATH, slope_constraint="symmetric", window=window)
            combined = np.concatenate((np.vstack(path1), np.vstack(path2 + random_points[i])),
                                      axis=1)
            if verbose:
...
def discriminative_guided_warp(x, labels, batch_size=6, slope_constraint="symmetric",
    ...
    for i, pat in enumerate(x):
        ...
        if positive.size > 0 and negative.size > 0:
            ...
            # Time warp
            warped = pat[path[1]]
            warp_path_interp = np.interp(orig_steps,
                                         np.linspace(0, x.shape[1] - 1., num=warped.shape[0]),
                                         path[1])
